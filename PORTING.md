...
src/kit/                    <- copy wholesale, do not edit per hub
  ...
  components/*.astro        <- Field, SelectField, Segmented, Chart,
                               AdSlot, DataNote, ToolShell, RelatedTools
...
```astro
<ToolShell title="..." intro="..." breadcrumbs={[...]}
           adSlotTop="x-top" adSlotResults="x-results" calcId="x">
  ...
  <AdSlot unit="x-mid" variant="inline" />
...
1. **Never name an AdSlot prop `slot`.** `slot` is Astro's reserved
   slot-assignment attribute. `<AdSlot slot="x-mid" />` inside a component's
   children is silently discarded — it renders nothing, no error. The prop is
   called `unit` for this reason.
...
Three units per tool page, heights reserved in CSS before any script runs:

- `adSlotTop` — above the calculator, `lazy={false}`
- `adSlotResults` — directly under the results card, highest CTR on a tool page
- one `<AdSlot unit="..." variant="inline" />` placed by the page mid-content

Never let a unit size itself. `--ad-h-*` in `tokens.css` fixes the container,
